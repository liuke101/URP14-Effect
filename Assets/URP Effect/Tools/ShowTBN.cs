...
public class ShowTBN : MonoBehaviour
{
     ...
     * color 向量颜色
     * */
    void DrawVectors (Vector3[ ] vertices, Vector3[ ] vectors, ref Matrix4x4 vertexMatrix, ref Matrix4x4 vectorMatrix, Color color, float vectorLen)
    {
        Gizmos.color = color;
        int len = (vertices == null || vectors == null ? 0 : vertices.Length);
        len = Mathf.Min (len, maxShowNum);
        if (vertices.Length != vectors.Length)
        {
            Debug.LogError ($"vertices length not equal vectors length, vetices length: {vertices.Length}, vectors length: {vectors.Length}");
            return;
        }
        for (int i = 0; i < len; i++)
        {
