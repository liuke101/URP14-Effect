...
public class LuminanceTextureRenderPass : ScriptableRenderPass
{
    ...
    // 构造函数
    //------------------------------------------------------
    public LuminanceTextureRenderPass(string commandBufferTag, string profilerTag, RenderPassEvent renderPassEvent,
    ...
    public void SetRenderPass(RTHandle cameraColorTargetHandle, int iterations, float blurRadius, int downSample,
        float luminanceThreshold, float lightDarkIntensity, Material waterMaterial)
    {
    ...
    {
        RenderingUtils.ReAllocateIfNeeded(ref m_tempRT0, m_rtDescriptor, FilterMode.Bilinear, name: "_TempRT0");

        RenderingUtils.ReAllocateIfNeeded(ref m_tempRT1, m_rtDescriptor, FilterMode.Bilinear, name: "_TempRT1");
