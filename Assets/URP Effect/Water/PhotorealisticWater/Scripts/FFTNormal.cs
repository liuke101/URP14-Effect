...
public class FFTNormal : MonoBehaviour
{
    [Range(3, 14)]
    ...
    public ComputeShader computeShader;   //计算水面的ComputeShader
    public Material waterMaterial;  //渲染水面的材质
    ...
    //ShaderPropertyID
    private static readonly int s_Displace = Shader.PropertyToID("_FFTDisplace");
    private static readonly int s_Normal = Shader.PropertyToID("_FFTNormal");
    private static readonly int s_Bubbles = Shader.PropertyToID("_FFTBubbles");
    ...
    {
        //设置海洋材质纹理
         waterMaterial.SetTexture(s_Displace, m_displaceRT);
         waterMaterial.SetTexture(s_Normal, m_normalRT);
         waterMaterial.SetTexture(s_Bubbles, m_bubblesRT);
    }
}
